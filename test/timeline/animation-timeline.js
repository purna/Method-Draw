...
var defaultTimelineOptions = {
  ...
  /**
   * Total timeline length in units (ms). When set, the track is extended to
   * at least this length so the full duration is visible even if the
   * keyframes do not span the whole range.
   */
  totalTime: 5000
};
...
var Timeline = /*#__PURE__*/function (_TimelineEventsEmitte) {
  ...
  function Timeline() {
    ...
    timeline_defineProperty(_this, "_generateViewModel", function () {
      ...
      // Extend the visible track to the configured total length so the full
      // duration is represented even when keyframes don't span the whole range.
      var totalTime = _this._options.totalTime;
      if (TimelineUtils.isNumber(totalTime) && (totalTime || totalTime === 0)) {
        if (!TimelineUtils.isNumber(toReturn.max) || totalTime > toReturn.max) {
          toReturn.max = totalTime;
        }
        toReturn.size.width = _this.valToPx(toReturn.max);
      }
      return toReturn;
    ...
    timeline_defineProperty(_this, "_renderBackground", function () {
      ...
      if (_this._options.fillColor && _this._options.fillColor !== 'transparent') {
        _this._ctx.save();
        _this._ctx.beginPath();
        ...
        // Clear if bg not set (or explicitly transparent so the CSS
        // background shows through). clearRect is required because filling
        // with a transparent color does not erase previously drawn content,
        // which would otherwise leave ghost trails when dragging the playhead.
