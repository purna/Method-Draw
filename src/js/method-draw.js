window.methodDraw = function () {
  ...
  Editor.setConfig = function (opts) {
    // full deep merge
    jQuery.extend(true, curConfig, opts);
    if (opts.extensions) {
  ...
  Editor.init = function () {
    ...
    // Wire up boolean operation buttons
    $('#join_flyout, #join_tool_menu').on('click', '.tool_button', function () {
      var op = $(this).data('mode');
      if ($(this).hasClass('disabled')) return;
      // Set the current class
      $(this).addClass('current').siblings().removeClass('current');
      // Set the main button as current
      $('#tool_join').addClass('current');
      svgCanvas.applyBooleanOperation(op);
    });

    // --- Boolean submenu flyout handling (from shapes-submenu-test.html) ---
    (function () {
      var menuBar = $('#menu_bar');
      var menu = $('#join_menu');
      var title = $('#join_title');
      var trigger = $('#join_trigger');
      var flyout = $('#join_flyout');
      ...
      trigger.on('mouseenter', function () {
        if (menu.hasClass('open')) flyout.show();
      });
    ...
    // --- Left side-toolbar boolean submenu ---
    (function () {
      var sideBtn = $('#tool_join');
      var sideMenu = $('#join_tool_menu');

      function show() { sideMenu.show(); }
      function hide() { sideMenu.hide(); }

      sideBtn.on('mouseenter', show);
      sideMenu.on('mouseenter', show);

      sideBtn.on('mouseleave', function (e) {
        if (!sideMenu[0].contains(e.relatedTarget)) hide();
      });
      sideMenu.on('mouseleave', function (e) {
        if (!sideBtn[0].contains(e.relatedTarget)) hide();
      });
    })();

    // Add boolean operation functionality to svgCanvas
    $.SvgCanvas.prototype.applyBooleanOperation = function (op) {
      var elems = this.getSelectedElems();
      if (elems.length < 2) {
        $.alert("Please select at least two paths to apply a boolean operation.");
        return;
      }

      var paths = elems.filter(Boolean).map(elem => {
        if (elem.tagName !== 'path') {
          return this.convertToPath(elem);
        }
        return elem;
      }).filter(Boolean);

      if (paths.length < 2) {
        $.alert("Boolean operations require at least two path elements.");
        return;
      }

      var pathElementToRings = function (elem) {
        if (!elem || typeof elem.getTotalLength !== 'function') return null;
        var rings = [];
        var len = elem.getTotalLength();
        if (!len) return null;
        // Flatten the path into a polygon by sampling points along its length.
        var steps = Math.max(8, Math.ceil(len / 2));
        var ring = [];
        for (var i = 0; i <= steps; i++) {
          var pt = elem.getPointAtLength((len * i) / steps);
          ring.push([pt.x, pt.y]);
        }
      ...
      var polygonsToD = function (rings) {
      ...
      var subjectRings = pathElementToRings(paths[1]);
      var clipRings = pathElementToRings(paths[0]);

      if (!subjectRings || !clipRings) {
        $.alert("Could not parse path data. Boolean operations support simple paths only.");
        return;
      }

      var resultRings = svgedit.boolop[op](subjectRings, clipRings);
      var newD = polygonsToD(resultRings);

      var batchCmd = new svgedit.history.BatchCommand("Boolean " + op);

      if (newD) {
        var subject = paths[1];
        batchCmd.addSubCommand(new svgedit.history.ChangeElementCommand(subject, {"d": newD}, "Boolean " + op));
        subject.setAttribute("d", newD);
        this.selectOnly([subject]);
      }

      this.undoMgr.addCommandToHistory(batchCmd);
    };
    ...
    Editor.selectedChanged = selectedChanged;
    ...
    var clickJoin = function () {
      if (toolButtonClick('#tool_join')) {
        svgCanvas.setMode('join');
      }
    };
    ...
    var Actions = function () {
      ...
      var tool_buttons = [
        ...
        { sel: '#tool_shapelib', fn: clickShapelib, evt: 'click' },
        { sel: '#tool_path', fn: clickPath, evt: 'click', key: ['P', true] },
        { sel: '#tool_text', fn: clickText, evt: 'click', key: ['T', true] },
        { sel: '#tool_join', fn: clickJoin, evt: 'click' },
        ...
        { sel: '#tool_export', fn: editor.export, evt: 'mouseup' },
        { sel: '#tool_export_json', fn: function () { editor.json.exportJson() }, evt: 'click' },
    ...
    var updateCanvas = Editor.updateCanvas = function (center, new_ctr) {
      ...
      if (curConfig.showRulers) {
        if (editor.rulers) editor.rulers.update(zoom);
        workarea.scroll();
      }
    }
    ...
    $(function () {
      ...
      svgCanvas.bind("changed", function (win, elems) {
        // updateContextPanel();
      });
      ...
      // Wire up timeline controls
      $('#tool_timeline_play').on('click', function () { methodDraw.timeline.onPlayClick(); });
      $('#tool_timeline_pause').on('click', function () { methodDraw.timeline.onPauseClick(); });
      $('#tool_timeline_loop').on('click', function () { methodDraw.timeline.toggleLoop(this); });
      $('#timeline_duration_input').on('change', function () { methodDraw.timeline.setTimelineDuration(this.value); });
      $('#tool_timeline_select').on('click', function () { methodDraw.timeline.selectMode(); });
      $('#tool_timeline_pan').on('click', function () { methodDraw.timeline.panMode(true); });
      $('#tool_timeline_pan_static').on('click', function () { methodDraw.timeline.panMode(false); });
      $('#tool_timeline_zoom').on('click', function () { methodDraw.timeline.zoomMode(); });
      $('#tool_timeline_none').on('click', function () { methodDraw.timeline.noneMode(); });
      $('#tool_timeline_add_keyframe').on('click', function () { methodDraw.timeline.addKeyframe(); });
      $('#tool_timeline_delete_keyframe').on('click', function () { methodDraw.timeline.removeKeyframe(); });
      $('#tool_timeline_collapse').on('click', function () { methodDraw.timeline.toggleTimelinePanel(); });
      $('#outline-scroll-container').on('wheel', function (e) { methodDraw.timeline.outlineMouseWheel(e.originalEvent); });

      // Initialize timeline
      methodDraw.timeline.init();
    });
  ...
  var loadSvgString = Editor.loadSvgString = function (str, callback) {
